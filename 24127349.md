# 🧭Interest + Activities Recommendation Pipeline – Smart Tourism System

## 1️⃣ Objective

This pipeline processes users' **intentions and preferences** – for example:

> “Tôi muốn đến một nơi yên tĩnh, có nhiều cây xanh, khí hậu mát mẻ và có thể tắm biển”

The objective is to **find the most emotionally and activity-aligned travel destinations** in the database (DB) using the model **SentenceTransformer + TF-IDF + Cosine Similarity + KNN**.

---

## 2️⃣ Pipeline Overview

```text
User enters vibe
↓
Vectorization (TF-IDF + SentenceTransformer)
↓
Similarity calculation (Cosine Similarity)
↓
Find top nearest locations (KNN / Ranking)
↓
Return relevant location suggestions
```

---

## 3️⃣ Detailed Components

### 🔹 3.1 TF-IDF (Term Frequency - Inverse Document Frequency)

TF-IDF evaluates the importance of a word in a text corpus.

$$
TF\text{-}IDF(t, d) = TF(t, d) \times \log\left(\frac{N}{DF(t)}\right)
$$

- \(TF(t, d)\): frequency of word \(t\) in document \(d\)  
- \(DF(t)\): number of documents containing word \(t\)  
- \(N\): total number of documents  

TF-IDF helps the model identify **distinctive keywords** for each location.
---

### 🔹 3.2 SentenceTransformer (Semantic Embedding)

#### 🧠 Overview

**SentenceTransformer** is a deep learning model based on **BERT (Bidirectional Encoder Representations from Transformers)**, fine-tuned to generate vector representations (embeddings) for entire sentences or paragraphs — rather than individual words.

The model aims to map sentences with **similar meanings** to **points close to each other in vector space**.

#### 🔬 General Structure
```text
Input string (sentence)
↓
Tokenizer (converts words → token IDs)
↓
Transformer Encoder (BERT / MiniLM / DistilBERT)
↓
Pooling Layer (Mean / Max / CLS token)
↓
Sentence Embedding (semantic vector)
```

Each sentence after passing through the model is represented by a vector with 384–768 dimensions (depending on the model), for example, `multilingual-e5-base` creates a 384-dimensional vector.

#### ⚙️ Detailed Working Mechanism

1. **Tokenizer**  
   Converts the input sentence into a string of token IDs, for example:  
   `“Đà Lạt yên bình” → [101, 3912, 1652, 102]`  
   (encoding based on WordPiece Tokenization)

2. **Transformer Encoder**  
   Applies *multi-head self-attention*, allowing the model to capture relationships between words in a bidirectional context:

  `Attention(Q, K, V) = softmax( (QKᵀ) / √dₖ ) × V`

   Where:
   - \(Q, K, V\): query, key, value matrices  
   - \(d\_k\): hidden size

3. **Pooling Layer**  
   Averages (mean pooling) all token embeddings into a single vector:

`s = (1/n) Σᵢ₌₁ⁿ hᵢ`


where \(h\_i\) is the embedding of the *i*th token.

4. **Sentence Embedding**  
   The result is a semantic vector representing the entire sentence, for example:
   [0.123, -0.041, 0.332, ... , 0.027]
   
#### 🧩 In the Vibe Recommendation Problem

In smart tourism systems, SentenceTransformer helps:
- Understand the **deep meaning** of location descriptions (“misty city” ≈ “cool weather, foggy”).
- Understand **abstract vibes** from users (“healing,” “peaceful,” “chill”).
- Connect different expressions with the same emotional content.

Example:

| User vibe | Location vibe | Cosine Similarity |
|----------------- |---------------|------------------|
| “yên tĩnh, khí hậu mát mẻ” | “không khí trong lành, nhiều cây thông” | 0.89 |
| “náo nhiệt, sôi động” | “biển, tiệc, lễ hội” | 0.86 |
| “chữa lành” | “spa, thiên nhiên, tĩnh tâm” | 0.81 |

The vector embeddings are normalized and used to calculate **cosine similarity**, enabling the system to suggest travel destinations that match the *mood* rather than just relying on *keywords*.
#### 💡 Mô hình sử dụng

In the current pipeline:
```python
from sentence_transformers import SentenceTransformer
st = SentenceTransformer(“multilingual-e5-base”)
```
---

## 4️⃣ Standardizing the hybrid pipeline (TF-IDF + SentenceTransformer)

To leverage the strengths of both models — TF-IDF (understanding keywords) and SentenceTransformer (understanding semantics) — we combine them into a hybrid vector:

**Combination formula:**

`Vₕᵧᵇʳᶦᵈ = [ V_TFIDF ; V_ST ]`

That is, we concatenate the two vectors horizontally.  
For example, if TF-IDF produces a 5000-dimensional vector and SentenceTransformer produces a 384-dimensional vector, then the hybrid vector has a total of 5384 dimensions.

---

## 5️⃣ Cosine Similarity

A common similarity measure in vector space is defined by **cosine similarity** — measuring the angle between two vectors.

**Formula:**

`cosine_similarity(A, B) = (A ∙ B) / (‖A‖ × ‖B‖)`

Values:
- 1 → identical  
- 0 → unrelated  
- -1 → opposite  

---

## 6️⃣ KNN (K-Nearest Neighbors)

KNN finds the **k nearest points** to the input vector in cosine space.

`d₍cos₎(A, B) = 1 − cosine_similarity(A, B)`

→ Select the **k locations with the smallest distance** to suggest.
---







